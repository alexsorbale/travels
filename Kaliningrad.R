...
Kaliningrad <- subset(Russia, NL_NAME_1 == "Калининградская область") 
...
Kaliningrad@data$Days <- c('не был', 'не был', 'не был', '16-21 июля', 'не был', 'не был', 'не был', 'не был', 'не был', 'не был', 'не был', 'не был', 'не был', 'не был', 'не был','не был','18-19 июля') 
...
Kaliningrad_to_map$Days <- factor(Kaliningrad_to_map$Days, levels = c("16-21 июля", "18-19 июля", "не был"))
...
kaliningrad_places <- read.csv("C:/Users/alexs/Desktop/Путешествия Р/Kaliningrad.csv", sep=';', stringsAsFactors = FALSE)
...
Kaliningrad_points<-Kaliningrad_final+geom_point(data=kaliningrad_places_1, aes(x = lon, y = lat, shape=group, color=group))+
  ...
  scale_shape_manual(name = "Обозначения", values = c(8, 16, 10), labels = c("Заповедники и парки","Населённые пункты", "Укрепления"))+
  scale_color_manual(name = "Обозначения", values = c('darkgreen', '#970027', 'black'), labels = c("Заповедники и парки","Населённые пункты", "Укрепления"))
Kaliningrad_points
